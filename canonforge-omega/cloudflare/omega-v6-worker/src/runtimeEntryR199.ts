...
async function statusR199(request: Request, env: any, id: string) {
  ...
  const heartbeatStale = ageMs !== null && !heartbeatCurrent;
...
async function hybridDirectR199(request: Request, env: any): Promise<Response | null> {
...
export default {
  async fetch(request: Request, env: any, ctx: any): Promise<Response> {
    const url = new URL(request.url);
    const direct = await hybridDirectR199(request, env);
    if (direct) return direct;
    const response = await current.fetch(request, env, ctx);
    return enhanceSovereignPcSurfaceR199(response, url.pathname);
  },
};
