...
export const SWARM_MODEL_R170 = "@cf/meta/llama-3.3-70b-instruct-fp8-fast";
export const SWARM_MODEL_R170_AUTHORITY = "Cloudflare-hosted Workers AI model output is MODEL_SYNTHESIS_NOT_CANON and never upgrades evidence authority or implies native PC execution.";
...
async function runCellTask(env: SwarmEnv, task: AnyObj, address: AnyObj, profile: AnyObj): Promise<AnyObj> {
  ...
  if (executor === "OPTICAL_CHAIN") {
    ...
    const proposalResponse = await env.OMEGA_GENESIS_MACHINE.fetch(new Request("https://genesis.internal/api/federation/propose", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ intent: clip(task.intent || task.text, 4000), evidence: evidence(task.evidence), ceremony_id: task.missionId || task.taskId || "swarm-r170" }) })), proposed = await proposalResponse.json().catch(() => null) as AnyObj | null, proposal = proposed?.packet;
