...
export const HYBRID_RETURN_ADMISSION_R203 = "r203-authenticated-return-admission";
...
/**
 * R203 hardens only the historical OMEGA_RUNTIME Hybrid return boundary.
 *
 * It intentionally extends the recovered R32/R33 runtime instead of replacing its
 * storage contract. The separate R201 durable mission evidence namespace and the
 * canonical R202 continuity-potential layer remain unchanged. A connected/authenticated
 * agent is not enough to complete a job: success requires an explicit outcome plus a
 * proof row for every queued step, ordered against the original step IDs/operations.
 * The server hashes the complete admission core itself. A host canonical receipt, when
 * supplied, is independently checked and raises the verification tier, but a host-provided
 * hash is never treated as authority by itself.
 */
export class OmegaRuntime extends BaseOmegaRuntime {
  async state() {
    ...
    return {
      ...
      returnAdmissionR203: {
        ...
        truthBoundary: "R203 proves authenticated return admission and server-side receipt integrity. It does not convert an authenticated software agent into hardware attestation or fabricate execution evidence absent returned step proofs.",
      },
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === "/agent/result" && request.method === "POST") {
      ...
      if (errors.length) {
        ...
        jobs = jobs.map(row => row.id === jobId ? {
          ...
          lifecycle: appendLifecycle(row, "RETURN_REJECTED_R203", { deviceId, serverReceiptSha256 }),
        } : row);
        ...
        await this.event("JOB_RETURN_REJECTED", `Job ${jobId} failed R203 authenticated return admission.`, {
      ...
      const packet = {
        schema: "OMEGA_HYBRID_RETURN_PACKET_R203",
        release: HYBRID_RETURN_ADMISSION_R203,
      ...
      await this.put("jobs", jobs);
      await this.event("JOB_RETURN_VERIFIED", `Job ${jobId} passed R203 return admission as ${finalStatus}.`, {
        jobId,
        deviceId,
        finalStatus,
      ...
      if (missionChanged) {
        ...
        await this.event(
          ...
          mission?.status === "COMPLETE"
            ? `Mission ${mission.id} completed only after R203 verified successful return admission.`
    ...
    if (path.startsWith("/jobs/") && path.endsWith("/receipt") && request.method === "GET") {
      ...
      return json({
        ok: true,
        schema: "OMEGA_HYBRID_JOB_RECEIPT_R203",
        release: HYBRID_RETURN_ADMISSION_R203,
        jobId,
        status: job.status,
