...
def test_r215_preserves_canonical_r214_earth_residual_markup_repair():
    nav = (SRC / "system" / "oneSystemNavigationR195.ts").read_text()
    assert "</div>';grid.appendChild(card);const esc=v=>String(v??'');" in nav
    assert "</div>';\ngrid.appendChild(card);const esc=v=>String(v??'');" not in nav
