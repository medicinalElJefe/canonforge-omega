...
def test_repair_is_additive_and_dispatcher_bound():
...
def test_successful_but_invalid_contract_still_hard_fails():
    repair = source()
    # The repair only reclassifies FAILED_RETURN/FAILED_INVOCATION when the HTTP
    # status is in the explicit transient external set. A 200 response that fails
    # semantic verification remains FAILED_* and therefore counts as a hard failure.
