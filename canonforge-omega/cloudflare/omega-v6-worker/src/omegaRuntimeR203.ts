  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === "/agent/result" && request.method === "POST") {
      ...
      jobs = jobs.map(row => row.id === jobId ? {
        ...
        outputPaths: normalized.outputs,
        log: packet.log,
        lifecycle: [...appendLifecycle(row, returnedState, { deviceId, serverReceiptSha256 }), { state: verification.state, at: completedAt, deviceId, serverReceiptSha256 }].slice(-40),
      } : row);
      await this.put("jobs", jobs);
      ...
      if (missionChanged) {
        ...
        await this.event(
          ...
          mission?.status === "COMPLETE" ? `Mission ${mission.id} completed only after R203 verified successful return admission.` : `Mission ${mission?.id} held after a verified failed host return.`,
