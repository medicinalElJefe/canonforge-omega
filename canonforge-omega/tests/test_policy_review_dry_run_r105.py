...
def test_r105_review_contract_and_decisions():
...
def test_r105_approval_only_creates_simulation_envelope():
    s=(SRC/'policyReviewDryRun.ts').read_text()
...
def test_r105_exposed_and_prior_schema_identities_preserved():
...
def test_r105_coexists_with_current_capability_inspector():
    s=(SRC/'capabilityRouter.ts').read_text()
    assert 'OMEGA_V6_CAPABILITY_INSPECTOR_V1' in s
    assert 'CAPABILITY_INSPECTOR_BOUNDARY' in s
