...
const convergenceCockpit = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>OMEGA V6 · Convergence</title><style>body{margin:0;background:#05070b;color:#f4f7ff;font:15px/1.5 system-ui}.w{max-width:1100px;margin:auto;padding:28px}.c{border:1px solid #26364d;border-radius:18px;background:#0b1119;padding:18px;margin:12px 0}pre{white-space:pre-wrap;word-break:break-word;color:#aab7ca}.b{border:1px solid #34445e;border-radius:999px;padding:8px 12px;color:inherit;text-decoration:none}.truth{color:#aebbd0}.warn{color:#edc75f}</style></head><body><main class="w"><a class="b" href="/">← OMEGA</a><div class="c"><h1>Governed live convergence</h1><p class="truth">V6 remains operational release authority; Genesis remains discovery/evolution authority; the sovereign PC remains an authenticated executor rather than canonical truth.</p><p>${CONVERGENCE_COCKPIT_BOUNDARY}</p><p id="freshness">Waiting for evidence.</p><p id="heartbeatTruth" class="warn">current authenticated heartbeat not proven</p></div><div class="c"><pre id="p">Loading current V6 ⇄ Genesis ⇄ PC evidence…</pre></div></main><script>function age(iso){const t=Date.parse(iso||'');if(!Number.isFinite(t))return'unknown freshness';const s=Math.max(0,Math.round((Date.now()-t)/1000));return s<60?s+'s old':Math.round(s/60)+'m old'}async function l(){try{const r=await fetch('/api/convergence/edge',{cache:'no-store'}),d=await r.json();document.querySelector('#p').textContent=JSON.stringify(d,null,2);document.querySelector('#freshness').textContent='Evidence '+age(d.timestamp);const pc=d?.topology?.sovereign_pc||{};document.querySelector('#heartbeatTruth').textContent=pc.pc_online?'PC ONLINE · current authenticated heartbeat proven':pc.heartbeat_current?'Authenticated heartbeat current · PC authority still not promoted':'current authenticated heartbeat not proven'}catch(e){document.querySelector('#p').textContent=String(e);document.querySelector('#freshness').textContent='Evidence unavailable';document.querySelector('#heartbeatTruth').textContent='current authenticated heartbeat not proven'}}l();setInterval(l,5000)</script></body></html>`;
