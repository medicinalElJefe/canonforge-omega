...
function markup(){const primary=routes.slice(0,8).map(r=>`<a class="oscView" href="${r[2]}">${r[0]}</a>`).join("");const all=routes.map(r=>`<a href="${r[2]}"><b>${r[0]}</b>${r[1]}</a>`).join("");return `<section id="omegaSpatialCore" aria-label="OMEGA compact workspace switcher"><span class="oscLegacyPhrase">One living system. Many working views.</span><div class="oscBar"><div class="oscIdentity"><span class="oscMark"></span><span><b>OMEGA</b><small>ONE STATE · MANY TOOLS</small></span></div><nav class="oscViews" aria-label="Primary workspaces">${primary}</nav><div class="oscStatus"><span id="oscDot" class="oscDot"></span><span id="oscAuthority">CHECKING</span><button id="oscMore" class="oscMore" type="button">VIEWS</button></div></div><div class="oscDrawer" id="oscDrawer"><div class="oscDrawerGrid">${all}<p>${SPATIAL_COMMAND_CORE_BOUNDARY}</p></div></div></section>`}

const runtime=`<script id="omegaSpatialCommandCoreRuntime">(()=>{const core=document.querySelector('#omegaSpatialCore');if(!core)return;const drawer=core.querySelector('#oscDrawer'),more=core.querySelector('#oscMore'),dot=core.querySelector('#oscDot'),authority=core.querySelector('#oscAuthority');if(more)more.onclick=()=>drawer?.classList.toggle('open');async function refresh(){try{const r=await fetch('/api/convergence/edge',{cache:'no-store'}),d=await r.json(),ok=Boolean(d?.convergence?.authority_contract_ready);dot.className='oscDot '+(ok?'good':'warn');authority.textContent=ok?'ROUTE CONTRACT READY':'ROUTE CONTRACT INCOMPLETE'}catch{dot.className='oscDot warn';authority.textContent='EDGE UNREACHABLE'}}refresh();setInterval(refresh,5000)})();</script>`;
