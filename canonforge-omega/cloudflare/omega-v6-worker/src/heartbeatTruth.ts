...
import sovereignEnvironment from "./sovereignEnvironment";
...
const convergenceCockpit = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>OMEGA V6 · Convergence</title><style>body{margin:0;background:#05070b;color:#f4f7ff;font:15px/1.5 system-ui}.w{max-width:1100px;margin:auto;padding:28px}.c{border:1px solid #26364d;border-radius:18px;background:#0b1119;padding:18px;margin:12px 0}pre{white-space:pre-wrap;word-break:break-word;color:#aab7ca}.b{border:1px solid #34445e;border-radius:999px;padding:8px 12px;color:inherit;text-decoration:none}.truth{color:#aebbd0}.warn{color:#edc75f}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.metric{border:1px solid #26364d;border-radius:14px;padding:12px;background:#08101a}.metric b{display:block;font-size:1.1rem;margin-top:5px}@media(max-width:700px){.grid{grid-template-columns:1fr}}</style></head><body><main class="w"><a class="b" href="/">← OMEGA</a><div class="c"><h1>Governed live convergence</h1><p class="truth">V6 remains operational release authority; Genesis remains discovery/evolution authority; the sovereign PC remains an authenticated executor rather than canonical truth.</p><p>${CONVERGENCE_COCKPIT_BOUNDARY}</p><div class="grid"><div class="metric"><span>Manifest agreement</span><b id="manifestAgreement">UNPROVEN</b></div><div class="metric"><span>Hybrid truth</span><b id="heartbeatTruth" class="warn">current authenticated heartbeat not proven</b></div></div><p id="freshness">Waiting for evidence.</p></div><div class="c"><pre id="p">Loading current V6 ⇄ Genesis ⇄ PC evidence…</pre></div></main><script>function age(iso){const t=Date.parse(iso||'');if(!Number.isFinite(t))return'unknown freshness';const s=Math.max(0,Math.round((Date.now()-t)/1000));return s<60?s+'s old':Math.round(s/60)+'m old'}async function l(){try{const r=await fetch('/api/convergence/edge',{cache:'no-store'}),d=await r.json();document.querySelector('#p').textContent=JSON.stringify(d,null,2);document.querySelector('#freshness').textContent='Evidence '+age(d.timestamp);const pc=d?.topology?.sovereign_pc||{};document.querySelector('#heartbeatTruth').textContent=pc.pc_online?'PC ONLINE':pc.heartbeat_current?'Authenticated heartbeat current · PC authority still not promoted':'current authenticated heartbeat not proven';const m=d?.genesis?.manifest||{},c=d?.convergence||{};document.querySelector('#manifestAgreement').textContent=c.reciprocal_manifest_ready&&m.digest&&c.genesis_manifest_digest===m.digest?'AGREED':'UNPROVEN'}catch(e){document.querySelector('#p').textContent=String(e);document.querySelector('#freshness').textContent='Evidence unavailable';document.querySelector('#heartbeatTruth').textContent='current authenticated heartbeat not proven';document.querySelector('#manifestAgreement').textContent='UNPROVEN'}}l();setInterval(l,5000)</script></body></html>`;
...
async function repairGenesisTransport(body: any, env: BoundEnv): Promise<any> {
  ...
  if (manifestProbe?.reachable) {
    body.genesis.manifest = { reachable: true, status: manifestProbe.status, schema: manifest.schema || null, digest: manifest.manifest_digest || null, capability_ids: genome.capability_ids || [], mode_count: genome.mode_count ?? null, promotion_boundary: manifest.promotion_boundary || null, dimensional_boundary: manifest.dimensional_boundary || null, transport: "SERVICE_BINDING" };
    body.convergence.reciprocal_manifest_ready = Boolean(manifest.schema === "OMEGA_RECURSIVE_CONVERGENCE_MANIFEST_V2" && manifest.manifest_digest);
...
async function injectCockpitLink(response: Response): Promise<Response> {
  ...
  const link = '<a href="/convergence" style="position:fixed;right:14px;bottom:14px;z-index:9999;border:1px solid #3b4f71;border-radius:999px;background:#09111aee;color:#f4f7ff;padding:9px 12px;text-decoration:none;font:700 12px/1.2 system-ui">LIVE CONVERGENCE ↗</a>';
...
export default {
  async fetch(request: Request, env: BoundEnv): Promise<Response> {
    ...
    if (specialist.matched) {
      ...
      root.pathname = "/";
      root.search = "";
      const response = await sovereignEnvironment.fetch(new Request(root.toString(), { method: "GET", headers: request.headers }), env);
    ...
    if (url.pathname === "/") {
      const response = await sovereignEnvironment.fetch(request, env);
      return injectCapabilityLink(await injectCockpitLink(response));
    }
