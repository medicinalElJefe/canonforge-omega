import { OmegaRuntime as OmegaRuntimeBase } from "./omegaRuntime";
...
function normalizeOutputs(value: unknown): { outputs: string[]; errors: string[] } {
  ...
  for (const row of rows) {
    const normalized = relativePath(row);
    if (!normalized) errors.push("output path escaped the approved relative namespace");
...
/**
 * R201 is an additive admission layer over the live R32/R33 Durable Object.
 * It preserves the existing storage keys and routes while replacing the old
 * `body.ok => COMPLETE` shortcut with authenticated, full-step return proof.
 */
export class OmegaRuntime extends OmegaRuntimeBase {
  async state() {
    const base: any = await super.state();
    const jobs = await this.get<any[]>("jobs", []);
    const latestVerified = jobs.filter(row => row?.returnVerification?.verified).at(-1) || null;
    return {
      ...base,
      schema: "OMEGA_LIVING_RUNTIME_R201",
      serverRevision: "R201_VERIFIED_RETURN_ADMISSION",
      returnAdmission: {
        authenticatedBridgeRequired: true,
        explicitOutcomeRequired: true,
        fullStepLineageRequiredForSuccess: true,
        unsafeOutputPathsRejected: true,
        serverCanonicalReceiptRequired: true,
        hostCanonicalReceiptRecognized: true,
        hostFingerprintAloneIsAuthority: false,
        missionCompletionRequiresVerifiedReturn: true,
      },
      latestVerifiedReturn: latestVerified ? {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === "/agent/result" && request.method === "POST") {
      ...
      if (body.returnedState && text(body.returnedState) !== returnedState) errors.push("returnedState does not match ok outcome");
      ...
      if (proofs.length > expectedSteps.length) errors.push("returned more step proofs than were queued");
      ...
      if (success && expectedSteps.length === 0) errors.push("successful return cannot complete an empty job");
      ...
      if (hostCanonical) {
        if (new TextEncoder().encode(hostCanonical).byteLength > RECEIPT_LIMIT) {
          ...
          if (receipt) {
            ...
            const observedHostHash = await sha256Text(hostCanonical);
            if (!hostFingerprint || observedHostHash !== hostFingerprint) errors.push("host canonical receipt SHA-256 mismatch");
            ...
            if (hostSteps.length !== stepReceipts.length) {
              ...
              for (let index = 0; index < stepReceipts.length; index++) {
                ...
                if (text(hostStep.id) !== text(serverStep.id) || text(hostStep.op).toUpperCase() !== text(serverStep.op).toUpperCase() || Boolean(hostStep.ok) !== Boolean(serverStep.ok) || text(hostStep.proofSha256).toLowerCase() !== serverStep.proofSha256) {
                  errors.push(`host receipt step ${index + 1} digest mismatch`);
                }
              }
            }
            const hostOutputs = Array.isArray(receipt.outputPaths) ? receipt.outputPaths.map((value: unknown) => relativePath(value)) : [];
            if (hostOutputs.some((value: string | null) => !value) || JSON.stringify(hostOutputs) !== JSON.stringify(normalized.outputs)) errors.push("host receipt output path mismatch");
      ...
      const admissionCore = {
        schema: "OMEGA_SERVER_RETURN_ADMISSION_CORE_R201",
        jobId,
        deviceId,
      ...
      if (target.returnVerification?.verified) {
        ...
        return json({ ok: false, code: "RESULT_CONFLICT", existingServerReceiptSha256: target.returnVerification.serverReceiptSha256 }, 409);
      }
      ...
      if (errors.length) {
        ...
        jobs = jobs.map(row => row.id === jobId ? {
          ...
          returnVerification: {
            verified: false,
            schema: "OMEGA_RETURN_VERIFICATION_R201",
          ...
          lifecycle: appendLifecycle(row, "RETURN_REJECTED", { deviceId, serverReceiptSha256 }),
        } : row);
        ...
        missions = missions.map(mission => {
          ...
          return {
            ...
            heldAt: rejectedAt,
            needsReview: true,
            lastProof: jobs.find(row => row.id === jobId)?.returnVerification || null,
          };
        });
        if (missionChanged) await this.put("missions", missions);
        await this.event("JOB_RETURN_REJECTED", `Job ${jobId} failed R201 return admission.`, { jobId, deviceId, errors, serverReceiptSha256 });
      ...
      const verification = {
        verified: true,
        schema: "OMEGA_RETURN_VERIFICATION_R201",
        state: success ? "RETURN_VERIFIED_SUCCESS_R201" : "RETURN_VERIFIED_FAILURE_R201",
      ...
      const packet = {
        schema: "OMEGA_HYBRID_RETURN_PACKET_R201",
      ...
      await this.put("jobs", jobs);
      await this.event("JOB_RETURN_VERIFIED", `Job ${jobId} passed R201 return admission as ${finalStatus}.`, { jobId, deviceId, finalStatus, verificationTier: verification.verificationTier, serverReceiptSha256 });
      ...
      missions = missions.map(mission => {
        if (mission.currentJobId !== jobId) return mission;
        missionChanged = true;
        const complete = finalStatus === "COMPLETE" && verification.verified;
      ...
      if (missionChanged) {
        ...
        await this.event(
          ...
          mission?.status === "COMPLETE"
            ? `Mission ${mission.id} completed only after R201 verified return admission.`
