...
def test_r185_live_proof_checks_every_public_node_after_canonical_deploy():
    workflow = WORKFLOW.read_text(encoding="utf-8")
    assert 'OMEGA V6 R185 live 172-cloud proof' in workflow
    assert 'workflows: ["OMEGA V6 verify"]' in workflow
    assert 'seq -w 1 172' in workflow
    assert '/api/clouds/r185/manifest' in workflow
    assert '/api/clouds/r185/node/$n' in workflow
    assert '/cloud/$n' in workflow
    assert 'configuredNodes' in workflow
    assert '172' in workflow
