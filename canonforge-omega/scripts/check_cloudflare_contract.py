...
def release_lineage_guard() -> dict:
    """Hold automatic canonical production promotion unless the push is a proper merge.

    This compensates for an unprotected canonical branch at the deployment boundary.
    On canonical GitHub Actions push, the commit must have exactly two parents and
    parent[0] must be the push event's exact previous canonical head. PR/local checks
    remain validation-only and are not blocked by this promotion-specific rule.
    """
    enforced = (
