...
const SCHEMA = "OMEGA_POST_HYBRID_DEVELOPMENT_CONTINUITY_R217";
const RELEASE = "r217-post-hybrid-development-continuity";
...
function developmentFacts(hybrid: any, development: any) {
  ...
  const advancedAfterReturn = Boolean(
    provenLink &&
      ...
      active &&
      active.id &&
      active.id !== verified[verified.length - 1]?.id,
  );
...
export async function handlePostHybridDevelopmentR217(
  request: Request,
  env: any,
  ctx: any,
  nextFetch: RuntimeFetch,
  ...
  const isStatus = url.pathname === "/api/system/r217/status" || url.pathname === "/api/system/r217/status/";
  const isResume = url.pathname === "/api/system/r217/resume" || url.pathname === "/api/system/r217/resume/";
  ...
  if (!body || body.confirmed !== true) {
    return json({
      ...
      boundary: "resume is bounded to the existing allow-listed development loop and does not authorize arbitrary shell, source mutation, deployment, or promotion",
    }, 422);
  }
...
const R217_MARKER = "OMEGA_POST_HYBRID_DEVELOPMENT_SURFACE_R217";

export async function enhancePostHybridDevelopmentR217(response: Response): Promise<Response> {
  const contentType = response.headers.get("content-type") || "";
  if (!contentType.toLowerCase().includes("text/html")) return response;

  const html = await response.text();
  if (!html.includes('data-view="Hybrid"') || html.includes(R217_MARKER)) {
    return new Response(html, { status: response.status, statusText: response.statusText, headers: response.headers });
  }

  const injection = `<script id="${R217_MARKER}">(function(){
...
var card=document.createElement('div');card.id='omegaR217Continuity';card.className='job';card.innerHTML='<span>POST-HYBRID</span><span class="muted" id="omegaR217Message">Proving link → execution → return → next development stage…</span><b id="omegaR217State">CHECKING</b>';rail.insertBefore(card,rail.firstChild);
var button=document.createElement('button');button.className='btn primary';button.id='omegaR217Resume';button.textContent='Resume governed development';button.disabled=true;controls.appendChild(button);
var proof=document.createElement('details');proof.className='proof';proof.innerHTML='<summary>Post-Hybrid continuity proof</summary><pre id="omegaR217Proof">No R217 proof loaded.</pre>';var side=panel.querySelector('.hybridStage aside.panel');if(side)side.appendChild(proof);
function setState(d){var state=String(d&&d.state||'STATUS_UNAVAILABLE');var msg='Hybrid link is not yet proven.';if(state==='LINK_AUTHENTICATED')msg='Authenticated link proven. Development can resume on the bounded allow-list.';if(state==='DEVELOPMENT_QUEUED')msg='A governed development job is queued for the authenticated host.';if(state==='HOST_EXECUTING')msg='The authenticated host has leased or is running a governed development job.';if(state==='HOST_RETURNED')msg='Verified host evidence returned; waiting for the next bounded stage.';if(state==='DEVELOPMENT_ADVANCING')msg='Verified host evidence returned and a different next development stage is active.';if(state==='BLOCKED')msg='Development returned a blocked/failed terminal state. Inspect the returned evidence before continuing.';if(state==='LINK_STALE')msg='Heartbeat is stale; execution and development claims are withheld.';document.getElementById('omegaR217State').textContent=state;document.getElementById('omegaR217Message').textContent=msg;button.disabled=!(d&&d.eligibleToResume===true);document.getElementById('omegaR217Proof').textContent=JSON.stringify(d,null,2);}
async function load(){try{var r=await fetch('/api/system/r217/status',{cache:'no-store'});var d=await r.json();setState(d);}catch(e){setState({state:'STATUS_UNAVAILABLE',eligibleToResume:false,error:String(e)});}}
button.addEventListener('click',async function(){button.disabled=true;button.textContent='Resuming governed development…';try{var r=await fetch('/api/system/r217/resume',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({confirmed:true}),cache:'no-store'});var d=await r.json();setState(d);button.textContent=r.ok?'Development loop active':'Resume blocked';}catch(e){setState({state:'STATUS_UNAVAILABLE',eligibleToResume:false,error:String(e)});button.textContent='Resume failed';}setTimeout(function(){button.textContent='Resume governed development';load();},1800);});
load();setInterval(function(){if(document.body.contains(panel))load();},4000);
  ...
  headers.set("x-omega-r217-surface", "post-hybrid-development-continuity");
