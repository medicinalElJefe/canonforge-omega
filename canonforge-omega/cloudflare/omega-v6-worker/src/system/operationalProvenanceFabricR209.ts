export const OPERATIONAL_PROVENANCE_RELEASE_R209 = "r209-operational-provenance-fabric";
export const OPERATIONAL_PROVENANCE_SCHEMA_R209 = "OMEGA_OPERATIONAL_PROVENANCE_FABRIC_R209";
...
async function invoke(request: Request, env: any, ctx: any, canonicalFetch: CanonicalFetch, spec: typeof SOURCES[number]) {
  ...
  target.search = `?r209Proof=${Date.now()}-${Math.random().toString(16).slice(2)}`;
  ...
  try {
    ...
    return {
      ...
      evidenceClass: "UNAVAILABLE" as EvidenceClass,
      body,
...
async function manifest(env: any) {
  const core = {
    ok: true,
    schema: OPERATIONAL_PROVENANCE_SCHEMA_R209,
    release: OPERATIONAL_PROVENANCE_RELEASE_R209,
    canonicalGitSha: env?.CANONICAL_GIT_SHA ?? null,
    purpose: "One read-only provenance fabric across the already-established OMEGA runtime, Earth/SAR, archive corpus, modes/calculus, intelligence, cloud/swarm, solver, Hybrid, durable mission and proof layers.",
    authority: "CORRELATED_EVIDENCE_VIEW_NOT_NEW_RUNTIME_AUTHORITY",
    predecessor: "r208-physical-sovereign-acceptance-closure",
    ...
    provenanceClasses: {
      ...
      DERIVED: "Computed/relational output produced from declared inputs; not a direct physical observation.",
      AVAILABLE: "Capability/interface is present but the queried response does not independently prove execution or physical state.",
    ...
    routes: {
      manifest: "/api/system/r209/manifest",
      status: "/api/system/r209/status",
      query: "/api/system/r209/query?domain=earth",
    },
    canonicalMutation: false,
    hostStateMutation: false,
    promotionAuthorized: false,
...
async function status(request: Request, env: any, ctx: any, canonicalFetch: CanonicalFetch) {
  ...
  const sources = calls.map(call => ({ ...call, evidenceClass: classify(call.id, call), body: undefined }));
...
async function query(request: Request, env: any, ctx: any, canonicalFetch: CanonicalFetch) {
  ...
  if (!selected.length) return json({ ok: false, code: "R209_SOURCE_NOT_FOUND", domain: domain || null, id: id || null, canonicalMutation: false, promotionAuthorized: false }, 404);
  ...
  const results = calls.map(call => ({
    ...
    evidenceClass: classify(call.id, call),
    elapsedMs: call.elapsedMs,
    evidence: call.body,
  }));
  const core = {
    ok: results.every(result => result.ok),
    schema: "OMEGA_OPERATIONAL_PROVENANCE_QUERY_R209",
    release: OPERATIONAL_PROVENANCE_RELEASE_R209,
    canonicalGitSha: env?.CANONICAL_GIT_SHA ?? null,
    filter: { domain: domain || null, id: id || null },
    results,
...
export async function handleOperationalProvenanceR209(request: Request, env: any, ctx: any, canonicalFetch: CanonicalFetch): Promise<Response | null> {
  const url = new URL(request.url);
  if (!url.pathname.startsWith("/api/system/r209/")) return null;
  ...
  if (url.pathname === "/api/system/r209/manifest") return json(await manifest(env));
  if (url.pathname === "/api/system/r209/status") return json(await status(request, env, ctx, canonicalFetch));
  if (url.pathname === "/api/system/r209/query") return json(await query(request, env, ctx, canonicalFetch));
  return json({ ok: false, code: "R209_ROUTE_NOT_FOUND", canonicalMutation: false, promotionAuthorized: false }, 404);
}
