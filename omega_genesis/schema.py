...
class Address20736:
    ...
    def __post_init__(self) -> None:
        for name, value in (("domain", self.domain), ("phase", self.phase), ("regulation", self.regulation), ("lens", self.lens)):
            if not 1 <= int(value) <= 12: raise ValueError(f"{name} must be in 1..12")
    @property
    def index0(self) -> int:
        return (((self.domain - 1) * 12 + (self.phase - 1)) * 12 + (self.regulation - 1)) * 12 + (self.lens - 1)
    @property
    def state_id(self) -> int: return self.index0 + 1
    @classmethod
    def from_index0(cls, index: int) -> "Address20736":
        if not 0 <= index < 20736: raise ValueError("index must be in 0..20735")
        d, rem = divmod(index, 12 ** 3); p, rem = divmod(rem, 12 ** 2); r, l = divmod(rem, 12)
        return cls(d + 1, p + 1, r + 1, l + 1)
    @classmethod
    def from_state_id(cls, state_id: int) -> "Address20736":
        if not 1 <= state_id <= 20736: raise ValueError("state_id must be in 1..20736")
        return cls.from_index0(state_id - 1)
    def as_tuple(self) -> tuple[int, int, int, int]: return (self.domain, self.phase, self.regulation, self.lens)
...
class CanonicalMetrics:
    ...
    def __post_init__(self) -> None:
        for name in self.__dataclass_fields__:
            if not isinstance(getattr(self, name), (int, float)): raise TypeError(f"{name} must be numeric")
...
class CanonicalPacket:
    ...
    def canonical_dict(self) -> dict[str, Any]:
        return {
            ...
            "sources": [{**asdict(s), "evidence_class": s.evidence_class.value} for s in self.sources],
    ...
    def public_dict(self) -> dict[str, Any]:
        d = self.canonical_dict(); d["digest"] = self.digest; d["state_id"] = self.address.state_id; d["index0"] = self.address.index0
        return d
