...
Write-OmegaLog "launch requested root=$Root canonical_port=$Port"
...
if ($agentProcess) {
  ...
  # The generated launcher deliberately derives OMEGA_ROOT from its own directory.
  # Saving it in $Root binds the heartbeat proof to the same approved repository root
  # rather than accidentally treating Downloads/Desktop as the sovereign root.
