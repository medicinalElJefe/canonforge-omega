...
from omega_genesis.evolution import build_snapshot, candidate_decision, load_policy
...
def test_evolution_snapshot_compiles_backlog(tmp_path):
    ...
    assert snapshot["quality_vector"]["capability_total"] == 18
    assert any(row["status"] in {"GAP", "BLOCKED_EXTERNAL"} for row in snapshot["backlog"])


def test_candidate_requires_strict_improvement():
    baseline = {"quality_vector": {
        "manifest_integrity": 1, "provenance_integrity": 1, "live_core_capabilities": 10,
        "capability_total": 18, "objectives_achieved": 3, "weighted_progress": 0.3, "weighted_gap": 0.7,
    }}
    same = {"quality_vector": dict(baseline["quality_vector"])}
    assert candidate_decision(baseline, same)["status"] == "QUARANTINE"

    improved = {"quality_vector": {**baseline["quality_vector"], "objectives_achieved": 4, "weighted_progress": 0.4, "weighted_gap": 0.6}}
...
def test_candidate_rejects_regression_even_with_other_gain():
    ...
    candidate = {"quality_vector": {
        "manifest_integrity": 1, "provenance_integrity": 1, "live_core_capabilities": 9,
        "capability_total": 18, "objectives_achieved": 4, "weighted_progress": 0.4, "weighted_gap": 0.6,
    }}
