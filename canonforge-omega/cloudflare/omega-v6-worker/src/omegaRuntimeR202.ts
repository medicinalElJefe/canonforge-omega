...
export const HYBRID_RETURN_ADMISSION_R202 = "r202-authenticated-return-admission";
export const HYBRID_RETURN_VERIFICATION_SCHEMA_R202 = "OMEGA_HYBRID_RETURN_VERIFICATION_R202";
...
const json = (value: unknown, status = 200) =>
  new Response(JSON.stringify(value, null, 2), { status, headers: JSON_HEADERS });
...
function sameStringArray(a: unknown, b: string[]): boolean {
  ...
  for (let index = 0; index < b.length; index++) {
    if (relativePath(a[index]) !== b[index]) return false;
  }
  return true;
}

/**
 * R202 hardens only the historical OMEGA_RUNTIME Hybrid return boundary.
 *
 * It intentionally extends the recovered R32/R33 runtime instead of replacing its
 * storage contract. The separate R201 durable mission evidence namespace remains
 * unchanged. A connected/authenticated agent is not enough to complete a job:
 * success requires an explicit outcome plus a proof row for every queued step,
 * ordered against the original step IDs/operations. The server hashes the complete
 * admission core itself. A host canonical receipt, when supplied, is independently
 * checked and raises the verification tier, but a host-provided hash is never treated
...
export class OmegaRuntime extends BaseOmegaRuntime {
  async state() {
    ...
    return {
      ...base,
      serverRevision: HYBRID_RETURN_ADMISSION_R202,
      returnAdmissionR202: {
        schema: HYBRID_RETURN_VERIFICATION_SCHEMA_R202,
        ...
        truthBoundary: "R202 proves authenticated return admission and server-side receipt integrity. It does not convert an authenticated software agent into hardware attestation or fabricate execution evidence absent returned step proofs.",
      },
      latestVerifiedReturnR202: latestVerified ? {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === "/agent/result" && request.method === "POST") {
      ...
      if (body.returnedState !== undefined && text(body.returnedState) !== returnedState) {
        errors.push("returnedState does not match explicit ok outcome");
      }
      ...
      for (let index = 0; index < proofs.length; index++) {
        ...
        stepReceipts.push({
          index,
          id: proofId || expectedId || null,
          op: proofOp || expectedOp || null,
          ok: proof.ok === true,
          proofSha256: await sha256Text(canonicalJson(proof)),
        });
      }
      ...
      if (hostFingerprint && !/^[0-9a-f]{64}$/.test(hostFingerprint)) {
        errors.push("host resultFingerprint must be SHA-256 hex when supplied");
      }
      ...
      if (hostCanonical) {
        if (new TextEncoder().encode(hostCanonical).byteLength > MAX_HOST_RECEIPT_BYTES) {
          errors.push("host canonical receipt exceeds bounded size");
        } else {
          let receipt: any = null;
          try { receipt = JSON.parse(hostCanonical); }
          catch { errors.push("host canonical receipt is not valid JSON"); }

          if (receipt) {
            hostReceiptSchema = text(receipt.schema) || null;
            hostReceiptObservedSha256 = await sha256Text(hostCanonical);
            if (!hostFingerprint || hostReceiptObservedSha256 !== hostFingerprint) {
              errors.push("host canonical receipt SHA-256 mismatch");
            }
            ...
            if (hostSteps.length !== stepReceipts.length) {
              errors.push("host receipt step lineage length mismatch");
            } else {
              for (let index = 0; index < stepReceipts.length; index++) {
                const hostStep = hostSteps[index] || {};
                const serverStep = stepReceipts[index];
                if (
                  text(hostStep.id) !== text(serverStep.id) ||
                  text(hostStep.op).toUpperCase() !== text(serverStep.op).toUpperCase() ||
                  Boolean(hostStep.ok) !== Boolean(serverStep.ok) ||
                  text(hostStep.proofSha256).toLowerCase() !== serverStep.proofSha256
                ) {
                  errors.push(`host receipt step ${index + 1} digest mismatch`);
                }
              }
            }
      ...
      const admissionCore = {
        schema: "OMEGA_SERVER_RETURN_ADMISSION_CORE_R202",
        release: HYBRID_RETURN_ADMISSION_R202,
        jobId,
        deviceId,
      ...
      if (target.returnVerification?.verified === true) {
        if (target.returnVerification.serverReceiptSha256 === serverReceiptSha256) {
          return json({ ok: true, idempotent: true, job: target, returnVerification: target.returnVerification });
        }
        return json({
          ok: false,
          code: "RESULT_CONFLICT",
          existingServerReceiptSha256: target.returnVerification.serverReceiptSha256,
          receivedServerReceiptSha256: serverReceiptSha256,
        }, 409);
      }
      ...
      if (errors.length) {
        ...
        const verification = {
          ...
          schema: HYBRID_RETURN_VERIFICATION_SCHEMA_R202,
          release: HYBRID_RETURN_ADMISSION_R202,
          state: "RETURN_REJECTED_R202",
          errors,
          serverReceiptSha256,
          hostResultFingerprint: hostFingerprint || null,
          hostReceiptObservedSha256,
          hostReceiptVerified: false,
        };
        jobs = jobs.map(row => row.id === jobId ? {
          ...
          lifecycle: appendLifecycle(row, "RETURN_REJECTED_R202", { deviceId, serverReceiptSha256 }),
        } : row);
        ...
        await this.event("JOB_RETURN_REJECTED", `Job ${jobId} failed R202 authenticated return admission.`, {
          jobId, deviceId, errors, serverReceiptSha256,
        });
      ...
      const verification = {
        ...
        schema: HYBRID_RETURN_VERIFICATION_SCHEMA_R202,
        release: HYBRID_RETURN_ADMISSION_R202,
        state: success ? "RETURN_VERIFIED_SUCCESS_R202" : "RETURN_VERIFIED_FAILURE_R202",
      ...
      const packet = {
        schema: "OMEGA_HYBRID_RETURN_PACKET_R202",
        release: HYBRID_RETURN_ADMISSION_R202,
        ...
        resultFingerprint: hostFingerprint || null,
        serverAdmissionCore: admissionCore,
        verification,
      };
      ...
      jobs = jobs.map(row => row.id === jobId ? {
        ...
        status: finalStatus,
        completedAt,
        heldAt: finalStatus === "FAILED" ? completedAt : undefined,
        needsReview: finalStatus === "FAILED",
        returnPacket: packet,
        returnVerification: verification,
        outputPaths: normalized.outputs,
        log: packet.log,
        lifecycle: [
          ...appendLifecycle(row, returnedState, { deviceId, serverReceiptSha256 }),
          { state: verification.state, at: completedAt, deviceId, serverReceiptSha256 },
        ].slice(-40),
      } : row);
      await this.put("jobs", jobs);
      await this.event("JOB_RETURN_VERIFIED", `Job ${jobId} passed R202 return admission as ${finalStatus}.`, {
        jobId,
        deviceId,
        finalStatus,
      ...
      missions = missions.map(mission => {
        ...
        return {
          ...
          heldAt: complete ? undefined : completedAt,
      ...
      if (missionChanged) {
        ...
        await this.event(
          ...
          mission?.status === "COMPLETE"
            ? `Mission ${mission.id} completed only after R202 verified successful return admission.`
            : `Mission ${mission?.id} held after a verified failed host return.`,
          { missionId: mission?.id, jobId, serverReceiptSha256 },
        );
      }
      ...
      return json({
        ...
        mission: missionChanged ? missions.find(row => row.currentJobId === jobId) : undefined,
        returnVerification: verification,
      });
    }
    ...
    if (path.startsWith("/jobs/") && path.endsWith("/receipt") && request.method === "GET") {
      ...
      return json({
        ok: true,
        schema: "OMEGA_HYBRID_JOB_RECEIPT_R202",
        release: HYBRID_RETURN_ADMISSION_R202,
        jobId,
        status: job.status,
        lifecycle: job.lifecycle || [],
        returnPacket: job.returnPacket || null,
        returnVerification: job.returnVerification || null,
      });
    }
