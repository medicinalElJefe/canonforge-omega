...
export async function handleCapabilityRequest(request: Request, env: CapabilityEnv, loadEdge: EdgeLoader): Promise<Response | null> {
  ...
  if (url.pathname === "/api/convergence/capabilities") {
    return Response.json(await capabilityIndex(env, loadEdge), { headers: { "cache-control": "no-store", "x-omega-authority": "observation-only" } });
  }
...
export async function injectCapabilityLink(response: Response): Promise<Response> {
  return mutateHtml(response, body => {
    let next = body;
    if (!next.includes('href="/capabilities"')) next = next.replace("</body>", '<a href="/capabilities" style="position:fixed;right:14px;bottom:62px;z-index:9999;border:1px solid #4b6388;border-radius:999px;background:#08111ddd;color:#f4f7ff;padding:9px 12px;text-decoration:none;font:700 12px/1.2 system-ui;box-shadow:0 12px 34px #0008">CAPABILITY ROUTER</a></body>');
    if (!next.includes('href="/workbench"')) next = next.replace("</body>", '<a href="/workbench" style="position:fixed;right:14px;bottom:110px;z-index:9999;border:1px solid #6d5ca8;border-radius:999px;background:#0b1020dd;color:#f4f7ff;padding:9px 12px;text-decoration:none;font:700 12px/1.2 system-ui;box-shadow:0 12px 34px #0008">STATE WORKBENCH</a></body>');
    return next;
  });
}
