...
function age(iso){const t=Date.parse(iso||'');if(!Number.isFinite(t))return 'unknown freshness';const s=Math.max(0,Math.round((Date.now()-t)/1000));return s<60?s+'s old':Math.round(s/60)+'m old'}
async function load(){q('#freshness').textContent='Refreshing…';try{const r=await fetch('/api/convergence/edge',{cache:'no-store'});const d=await r.json();const c=d.convergence||{},g=d.genesis||{},m=g.manifest||{},pc=d.topology?.sovereign_pc||{},v6=d.topology?.v6?.edge||{},ge=d.topology?.genesis?.edge||{};const manifestOk=Boolean(c.reciprocal_manifest_ready&&m.digest&&c.genesis_manifest_digest===m.digest);const transport=c.genesis_transport||m.transport||'UNPROVEN';const mesh=Boolean(d.ok&&manifestOk);q('#transport').textContent=transport;q('#transport').className=transport==='SERVICE_BINDING'?'good':'warn';q('#manifestState').textContent=manifestOk?'AGREED':'UNPROVEN';q('#manifestState').className=manifestOk?'good':'warn';q('#manifestShort').textContent=m.digest?m.digest.slice(0,16)+'…':'no digest';q('#capCount').textContent=g.capability_count??'—';q('#modeCount').textContent=(m.mode_count??'—')+' modes';q('#pcState').textContent=pc.pc_online?'PC ONLINE':pc.heartbeat_current?'HEARTBEAT CURRENT':'PC UNPROVEN';q('#pcState').className=pc.pc_online?'good':pc.heartbeat_current?'warn':'warn';q('#heartbeatAge').textContent=pc.heartbeat_current?'heartbeat '+(pc.heartbeat_age_seconds??'—')+'s old':'current authenticated heartbeat not proven';q('#v6State').textContent=v6.reachable?'REACHABLE · '+(v6.status??''):'DEGRADED';q('#genesisState').textContent=ge.reachable?'REACHABLE · '+(ge.status??''):'DEGRADED';q('#pcDetail').textContent=(pc.state||'UNPROVEN')+' · online='+String(Boolean(pc.pc_online));q('#evidenceTime').textContent=d.timestamp||'—';q('#digest').textContent=m.digest||'—';q('#transportBoundary').textContent=c.genesis_transport_boundary||'No transport boundary reported.';q('#freshness').textContent='Evidence '+age(d.timestamp);q('#decision').textContent=mesh?'RECIPROCAL CONVERGENCE PROVEN':'CONVERGENCE DEGRADED';q('#decision').className=mesh?'good':'warn';q('#decisionWhy').textContent=mesh?'V6 can observe the current Genesis manifest through the governed transport and the digest contract is internally consistent.':'One or more current evidence gates are not satisfied.';q('#meshDot').className='dot '+(mesh?'good':'warn');q('#meshLabel').textContent=mesh?'Mesh proof current':'Mesh proof incomplete';const ids=m.capability_ids||[];q('#capabilities').innerHTML=ids.length?ids.map(x=>'<span class="cap">'+esc(x)+'</span>').join(''):'<span class="muted">No capability IDs reported.</span>';const gates=g.acceptance_gates||[];q('#gates').innerHTML=gates.length?gates.map(x=>'<span class="cap">'+esc(x)+'</span>').join(''):'<span class="muted">No acceptance gates reported.</span>';q('#devMode').textContent=d.development?.mode||'UNKNOWN';const a=d.development?.active_job;q('#activeJob').textContent=a?(a.kind||'job')+' · '+(a.reason||''):'None reported';const jobs=(d.development?.recent_jobs||[]).slice().reverse().slice(0,5);q('#jobs').innerHTML=jobs.map(j=>'<div class="row"><b>'+esc(j.kind||'job')+'</b><span>'+esc(j.state||'')+' · '+esc(j.reason||'')+'</span></div>').join('');q('#proof').textContent=JSON.stringify(d,null,2)}catch(e){q('#freshness').textContent='Evidence unavailable';q('#decision').textContent='CONVERGENCE UNREACHABLE';q('#decision').className='bad';q('#meshDot').className='dot bad';q('#meshLabel').textContent='Public proof request failed';q('#proof').textContent=String(e)}}
q('#refresh').onclick=load;load();setInterval(load,5000);
</script></body></html>`;
...
export default {
  async fetch(request: Request, env: BoundEnv): Promise<Response> {
    ...
    if (url.pathname === "/convergence") {
      return new Response(convergenceCockpit, {
        headers: {
    ...
    if (url.pathname === "/api/convergence/edge") return enforceHeartbeatTruth(response, env);
