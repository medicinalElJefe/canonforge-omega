...
import { enhanceSwarmPrecisionBody, SWARM_PRECISION_BODY_RELEASE } from "./swarmPrecisionBody";

export { VIRTUAL_LATTICE_BOUNDARY } from "./virtualLatticeDisplayCore";

/* Preserve historical release markers because accepted public verifiers key on them. */
...
export const SWARM_BODY_RELEASE = SWARM_PRECISION_BODY_RELEASE;
...
export const VISUAL_DELIVERY_BOUNDARY =
  "R170 preserves all R168 state/SAI/Hybrid truth boundaries and adds an operator-visible 1→12→144→1,728→20,736 swarm body driven by live planner and Durable Object mission responses. The field displays exact selected cell addresses returned by /api/swarm/autonomic/plan and live mission progress returned by the autonomic coordinator; idle cells are address capacity rather than claims of continuous compute. Workers AI provider use remains explicit and receipt-bound. Existing canonical state, route authority, Hybrid heartbeat, Earth observed-source, optical validation, and release-governance boundaries remain independent. 12^n shells are representation/execution-resolution levels and are not physical dimensions.";
...
async function stampDeliveredVisual(response: Response): Promise<Response> {
  ...
  if (!html.includes("omegaVisualDeliveryRelease")) html = html.includes("</head>") ? html.replace("</head>", meta + "</head>") : meta + html;
    ...
    const missing = [
      ...
      ["omegaSwarmBodyRelease", `<meta id="omegaSwarmBodyRelease" name="omega-swarm-body-release" content="${SWARM_BODY_RELEASE}">`],
    ] as const;
    for (const [id, tag] of missing) if (!html.includes(id)) html = html.replace("</head>", tag + "</head>");
  }
...
export async function enhanceVirtualLatticeDisplay(response: Response): Promise<Response> {
  ...
  rendered = await enhanceSwarmPrecisionBody(rendered);
