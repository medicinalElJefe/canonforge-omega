...
export const HYBRID_RETURN_ADMISSION_R204 = "r204-verified-hybrid-return-admission";
export const HYBRID_RETURN_VERIFICATION_SCHEMA_R204 = "OMEGA_HYBRID_RETURN_VERIFICATION_R204";
...
/**
 * R204 hardens the pre-existing OMEGA_RUNTIME /agent/result boundary only.
 * R203 mission continuity remains above this runtime and therefore receives stronger
 * returned host evidence without any new Durable Object namespace or storage migration.
 */
export class OmegaRuntime extends BaseOmegaRuntime {
  async state() {
    ...
    return {
      ...base,
      serverRevision: HYBRID_RETURN_ADMISSION_R204,
      returnAdmissionR204: {
        schema: HYBRID_RETURN_VERIFICATION_SCHEMA_R204,
        ...
        r203MissionContinuityPreserved: true,
        truthBoundary: "R204 proves authenticated software-return admission and server-side receipt integrity. It does not convert an authenticated software agent into hardware attestation or fabricate execution evidence absent returned step proofs.",
      },
      latestVerifiedReturnR204: latestVerified ? {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === "/agent/result" && request.method === "POST") {
      ...
      if (hostCanonical) {
        if (new TextEncoder().encode(hostCanonical).byteLength > MAX_HOST_RECEIPT_BYTES) errors.push("host canonical receipt exceeds bounded size");
          ...
          if (receipt) {
            ...
            if (typeof receipt.ok !== "boolean") errors.push("host receipt explicit boolean outcome required");
            ...
            if (receipt.ok !== success) errors.push("host receipt outcome mismatch");
      ...
      const admissionCore = {
        schema: "OMEGA_SERVER_RETURN_ADMISSION_CORE_R204",
      ...
      if (errors.length) {
        ...
        const verification = { verified: false, schema: HYBRID_RETURN_VERIFICATION_SCHEMA_R204, release: HYBRID_RETURN_ADMISSION_R204, state: "RETURN_REJECTED_R204", errors, serverReceiptSha256, hostResultFingerprint: hostFingerprint || null, hostReceiptObservedSha256, hostReceiptVerified: false, hostCryptographicAttestationClaimed: false };
        jobs = jobs.map(row => row.id === jobId ? { ...row, status: "RETURN_REJECTED", heldAt: rejectedAt, needsReview: true, returnVerification: verification, lifecycle: appendLifecycle(row, "RETURN_REJECTED_R204", { deviceId, serverReceiptSha256 }) } : row);
        ...
        await this.event("JOB_RETURN_REJECTED", `Job ${jobId} failed R204 authenticated return admission.`, { jobId, deviceId, errors, serverReceiptSha256 });
      ...
      const verification = {
        ...
        schema: HYBRID_RETURN_VERIFICATION_SCHEMA_R204,
        release: HYBRID_RETURN_ADMISSION_R204,
        state: success ? "RETURN_VERIFIED_SUCCESS_R204" : "RETURN_VERIFIED_FAILURE_R204",
      ...
      const packet = { schema: "OMEGA_HYBRID_RETURN_PACKET_R204", release: HYBRID_RETURN_ADMISSION_R204, receivedAt: completedAt, deviceId, executionId: text(body.executionId).slice(0, 160) || null, returnedState, stepProofs: proofs, outputPaths: normalized.outputs, log: text(body.log).slice(-12000), evaluation: body.evaluation ?? null, promotion: body.promotion ?? null, resultFingerprint: hostFingerprint || null, serverAdmissionCore: admissionCore, verification };
      ...
      await this.put("jobs", jobs);
      await this.event("JOB_RETURN_VERIFIED", `Job ${jobId} passed R204 return admission as ${finalStatus}.`, { jobId, deviceId, finalStatus, verificationTier: verification.verificationTier, serverReceiptSha256 });
      ...
      if (missionChanged) {
        ...
        await this.event(mission?.status === "COMPLETE" ? "MISSION_COMPLETE" : "MISSION_HELD", mission?.status === "COMPLETE" ? `Mission ${mission.id} completed only after R204 verified successful return admission.` : `Mission ${mission?.id} held after a verified failed host return.`, { missionId: mission?.id, jobId, serverReceiptSha256 });
      }
    ...
    if (path.startsWith("/jobs/") && path.endsWith("/receipt") && request.method === "GET") {
      ...
      return json({ ok: true, schema: "OMEGA_HYBRID_JOB_RECEIPT_R204", release: HYBRID_RETURN_ADMISSION_R204, jobId, status: job.status, lifecycle: job.lifecycle || [], returnPacket: job.returnPacket || null, returnVerification: job.returnVerification || null, authority: "AUTHENTICATED_EXECUTION_RETURN_EVIDENCE_NOT_CANONSTATE", canonicalMutation: false, promotionAuthorized: false });
    }
