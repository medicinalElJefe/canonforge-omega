...
$R210Archiver = Join-Path $PSScriptRoot 'ARCHIVE_OMEGA_SOVEREIGN_PROOF_R210.ps1'
...
if (-not (Test-Path $R210Archiver)) {
  throw "R210 proof archiver missing: $R210Archiver"
}
...
function Save-R210Archive([string]$Path, [string]$Kind, [int]$Attempt) {
  $archiveJson = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $R210Archiver -ReceiptPath $Path -Kind $Kind -Attempt $Attempt
  if ($LASTEXITCODE -ne 0) {
    throw "R210 archive command failed for $Kind attempt $Attempt"
  }
  $archive = ($archiveJson -join "`n") | ConvertFrom-Json
  if ([string]$archive.schema -ne 'OMEGA_SOVEREIGN_PROOF_ARCHIVE_RESULT_R210') {
    throw "Unexpected R210 archive result schema: $($archive.schema)"
  }
  return $archive
}
...
for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
  ...
  # Every attempt must bind to evidence created by that attempt. R210 archives each
  # validated fresh R208 receipt before the next attempt may replace the local latest file.
  ...
  if (Test-Path $R208ReceiptPath) {
    try {
      ...
      $attempts += [ordered]@{
        ...
        r208ExitCode = $exitCode
        fullAcceptance = $false
        acceptanceState = 'R208_RECEIPT_PARSE_FRESHNESS_OR_ARCHIVE_FAILED'
        blockers = @('R208_RECEIPT_PARSE_FRESHNESS_OR_ARCHIVE_FAILED')
