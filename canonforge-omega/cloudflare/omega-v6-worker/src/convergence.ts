...
async function convergenceSnapshot(request: Request, env: Env) {
  const [v6Edge, v6Hybrid, v6Development, genesisEdge, genesisHealth, genesisCapabilities, genesisModes] = await Promise.all([
  ...
  return {
    ...
    genesis: {
      capability_count: caps.count ?? (Array.isArray(caps.capabilities) ? caps.capabilities.length : null),
      acceptance_gates: caps.acceptance_gates || [],
      menus: caps.menus || [],
