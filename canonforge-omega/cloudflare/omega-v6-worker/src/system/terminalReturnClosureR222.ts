...
function currentJobIdentity(container?: any): string | null {
  const c = object(container) || {};
  return text(c.currentJobId ?? c.current_job_id ?? c.currentJob?.id ?? c.current_job?.id) || null;
}

function exactCurrentJobBinding(job: any, container?: any): boolean {
  const jid = identity(job);
  const current = currentJobIdentity(container);
  return Boolean(jid && current && jid === current);
}
...
function returnEvidence(job: any, container?: any): ReturnEvidenceR222 {
  ...
  if (exactCurrentJobBinding(job, container)) {
    ...
    if (lastProof?.schema === R204_VERIFICATION_SCHEMA || meaningfulObject(c.returnProof ?? c.return_proof) || meaningfulObject(c.verifiedReturn ?? c.verified_return)) {
      return { present: true, source: lastProof?.schema === R204_VERIFICATION_SCHEMA ? "R204_EXACT_JOB_BOUND_MISSION_LAST_PROOF" : "EXACT_JOB_BOUND_CONTAINER_RETURN_EVIDENCE" };
    }
  }
...
function r204Verification(job: any, container?: any): Record<string, any> | null {
  ...
  if (exactCurrentJobBinding(job, container)) candidates.push(c.lastProof, c.last_proof);
...
function primaryReconciliation(
  ...
  const sameIdentity = Boolean(development.jobId && hybrid.jobId && development.jobId === hybrid.jobId);
  ...
  if (sameIdentity) {
    ...
    if (development.terminal !== hybrid.terminal) return development.terminal
      ? { surface: "development", reconciliation: development }
      : { surface: "hybrid", reconciliation: hybrid };
  }
  ...
  if (url.pathname === "/api/system/r222/status") {
    ...
    return json({
      ...
      boundaries: {
        ...
        containerVerificationRequiresExactJobIdentity: true,
        sameJobNewerTerminalBeatsStaleInflightProjection: true,
