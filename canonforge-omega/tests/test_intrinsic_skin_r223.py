...
from omega_runtime.intrinsic_skin import (
    OmegaAddr, PathStep, PathAddress, ClosureAddress, ProofObligation, ClosureNavigator,
    SKINS, canonical_20736_skin, cross_skin_q, decide_resolution, skin_manifest,
)
from omega_runtime.state import Address20736, EvidenceClass, MotionState, StateEnvelope, StateMetrics


def envelope():
    return StateEnvelope(address=Address20736(2,3,4,5),evidence_class=EvidenceClass.DERIVED,
...
def test_r223_path_and_closure_are_first_class_addresses():
    steps=(PathStep("A","B","TRANSFORM",residual=.02),PathStep("B","A","RETURN",residual=.01))
    path=PathAddress("A",steps,20736,"canonical",1)
    closure=ClosureAddress(path,"canonical-equivalence",.01,.0,.03,.02,"APPROXIMATE")
    coord=OmegaAddr(Address20736(1,2,3,4),20736,"canonical",1.0).with_path(path,closure.key)
    assert path.end=="A" and len(path.key)==64 and len(closure.key)==64
    assert coord.path_address==path.key and coord.closure_address==closure.key
...
def test_r223_proof_obligations_navigate_unresolved_closure_space():
    nav=ClosureNavigator()
    p1=PathAddress("A",(PathStep("A","A","CHECK"),),144,"canonical")
    p2=PathAddress("B",(PathStep("B","B","CHECK"),),248832,"canonical")
    closed=ProofObligation("o1","invariant",ClosureAddress(p1,"eq",0,0,0,0,"EXACT"),priority=1)
    open_=ProofObligation("o2","return",ClosureAddress(p2,"eq",.4,.1,.2,.5,"FAILED"),priority=9,dependencies=("o1",))
    assert closed.resolved is True and open_.resolved is False
    assert nav.next((open_,closed),resolved_ids=("o1",)).obligation_id=="o2"
    assert nav.route(open_,epsilon=.1)=="TURN"
    assert nav.next((open_,closed),resolved_ids=()) is None
...
def test_r223_canonical_skin_binds_existing_state_geometry_motion_and_invariants():
    ...
    assert state.geometry["degree"]==7 and len(state.geometry["neighbors"])==7
    assert state.motion["v"]==.4 and len(state.invariants["canonical_digest"])==64
...
def test_r223_manifest_preserves_truth_boundary_and_receipt():
    m=skin_manifest()
    assert m["skins"]==list(SKINS) and m["physical_dimension_claim"] is False and m["canonical_mutation"] is False
