...
def test_r205_launcher_keeps_one_canonical_localhost_identity():
    text = read(LAUNCHER)
...
def test_r205_launcher_binds_pairing_launcher_to_approved_repository_root():
    text = read(LAUNCHER)
...
def test_r205_launcher_never_promotes_pending_heartbeat_to_online():
    text = read(LAUNCHER)
...
def test_r205_installer_requires_native_rcwa_and_persists_same_entry_point():
    text = read(INSTALLER)
