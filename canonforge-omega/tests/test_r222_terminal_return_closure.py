...
def test_r222_does_not_borrow_generic_container_verification_for_another_job():
    s = read(R222)
    assert 'sameCurrentJob(job, container)' in s
    assert 'containerVerificationDoesNotVerifyAnotherJob: true' in s
...
def test_r222_primary_status_prefers_current_activity_then_newer_terminal_evidence():
    s = read(R222)
...
def test_r222_integration_retains_latest_r221_native_rcwa_retry_without_retraining():
    r221 = read(R221)
