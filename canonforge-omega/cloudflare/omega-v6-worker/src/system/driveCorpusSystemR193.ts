import {
  DRIVE_CORPUS_SNAPSHOT_SHA256_R193,
  driveCorpusIntegrityR193,
  readDriveCorpusSnapshotR193,
} from "./driveCorpusSnapshotR193";

export const DRIVE_CORPUS_RELEASE_R193 = "r193-drive-corpus-one-system";
export const DRIVE_CORPUS_SCHEMA_R193 = "OMEGA_DRIVE_CORPUS_ONE_SYSTEM_R193";
export const MODE188_FORMULA_R193 = "S188=CΩ/(Λ+q+0.35Λq+0.05)";
...
const CORPUS_TABLES: readonly CorpusTable[] = [
  { id: "registry", aliases: ["registry", "softwareRegistry", "software_registry", "software"], label: "Software / Artifact Registry", expectedMinimum: 100 },
  { id: "menus", aliases: ["menus", "menuOptions", "menu_options", "menu"], label: "One-System Menu Options", expectedMinimum: 36 },
  { id: "capabilities", aliases: ["capabilities", "capabilityRows", "capability_rows", "capability"], label: "Capability Contract", expectedMinimum: 18 },
  { id: "stateSpaces", aliases: ["stateSpaces", "state_spaces", "statespaces", "stateSpace"], label: "State Spaces" },
  { id: "growthSequence", aliases: ["growthSequence", "growth_sequence", "growth", "buildSequence", "build_sequence"], label: "Growth / Build Sequence" },
  { id: "runtimeWiring", aliases: ["runtimeWiring", "runtime_wiring", "wiring", "runtime"], label: "Runtime Wiring", expectedMinimum: 16 },
  { id: "packageSetup", aliases: ["packageSetup", "package_setup", "packaging", "package"], label: "Package / Install Setup" },
  { id: "acceptanceGates", aliases: ["acceptanceGates", "acceptance_gates", "gates", "proofGates", "proof_gates"], label: "Acceptance Gates", expectedMinimum: 12 },
  { id: "capacityAddressIndex", aliases: ["capacityAddressIndex", "capacity_address_index", "addressIndex", "address_index", "capacity"], label: "Capacity Address Index", expectedMinimum: 144 },
  { id: "implementationSequence", aliases: ["implementationSequence", "implementation_sequence", "implementation", "milestones"], label: "Implementation Sequence", expectedMinimum: 16 },
  { id: "upgradeMap", aliases: ["upgradeMap", "upgrade_map", "upgrades", "upgrade"], label: "Upgrade Map" },
] as const;
...
export function mode188DerivedR193(row: Row) {
  ...
  return {
    ...
    score,
    sourceDecision,
    boundary: "R193 derives the documented Mode188 stability index from corpus metrics but does not invent STAY/TURN/ESCALATE thresholds when the source row does not provide them.",
  };
}
...
async function corpusManifestR193(env: any) {
  const [snapshot, integrity] = await Promise.all([readDriveCorpusSnapshotR193(), driveCorpusIntegrityR193()]);
  ...
  const tableSummary = CORPUS_TABLES.map(table => ({
    ...
    route: `/api/system/r193/${table.id}`,
  }));
  ...
  return {
    ...
    schema: DRIVE_CORPUS_SCHEMA_R193,
    release: DRIVE_CORPUS_RELEASE_R193,
    predecessorRelease: "r192-navigation-home-repair",
    canonicalGitSha: env?.CANONICAL_GIT_SHA ?? null,
    corpus: {
      embedded: true,
      losslessSnapshot: true,
      sha256: DRIVE_CORPUS_SNAPSHOT_SHA256_R193,
      integrity,
      tables: tableSummary,
      dispositions,
    },
    ...
    routes: {
      manifest: "/api/system/r193/manifest",
      status: "/api/system/r193/status",
      execute: "/api/system/r193/execute",
      analyze: "/api/system/r193/analyze?table=registry",
      tables: Object.fromEntries(CORPUS_TABLES.map(table => [table.id, `/api/system/r193/${table.id}`])),
      cockpit: "/system",
    },
...
async function liveStatusR193(request: Request, env: any, ctx: any, canonicalFetch: CanonicalFetch) {
  const [manifest, probes] = await Promise.all([
    corpusManifestR193(env),
  ...
  const core = {
    schema: "OMEGA_ONE_SYSTEM_LIVE_STATUS_R193",
    release: DRIVE_CORPUS_RELEASE_R193,
    canonicalGitSha: env?.CANONICAL_GIT_SHA ?? null,
...
async function executeR193(request: Request, env: any, ctx: any, canonicalFetch: CanonicalFetch): Promise<Response> {
  const body = await request.json().catch(() => ({})) as any;
  const operation = String(body?.operation ?? "").trim();
  const target = EXECUTION_TARGETS[operation];
  if (!target) {
    return json({ ok: false, code: "R193_OPERATION_NOT_ALLOWED", allowed: Object.keys(EXECUTION_TARGETS), canonicalMutation: false }, 422);
  }
  ...
  const receiptCore = {
    schema: "OMEGA_ONE_SYSTEM_EXECUTION_RECEIPT_R193",
    release: DRIVE_CORPUS_RELEASE_R193,
    operation,
    ...
    result,
    canonicalMutation: false,
    authority: "R193_ORCHESTRATES_EXISTING_SPECIALIST_AUTHORITY;IT_DOES_NOT_REIMPLEMENT_OR_OVERRIDE_THE_SPECIALIST",
  };
...
async function tableResponseR193(tableId: string, url: URL): Promise<Response> {
  const table = CORPUS_TABLES.find(candidate => candidate.id === tableId);
  if (!table) return json({ ok: false, code: "R193_TABLE_NOT_FOUND" }, 404);
  const snapshot = await readDriveCorpusSnapshotR193();
  const rows = tableFromSnapshot(snapshot, table);
  const page = filteredRows(rows, url);
  return json({
    ok: true,
    schema: "OMEGA_DRIVE_CORPUS_TABLE_R193",
    release: DRIVE_CORPUS_RELEASE_R193,
    table: table.id,
    label: table.label,
...
async function analyzeResponseR193(url: URL): Promise<Response> {
  const tableId = String(url.searchParams.get("table") ?? "registry");
  const table = CORPUS_TABLES.find(candidate => candidate.id === tableId);
  if (!table) return json({ ok: false, code: "R193_TABLE_NOT_FOUND", tables: CORPUS_TABLES.map(row => row.id) }, 404);
  const snapshot = await readDriveCorpusSnapshotR193();
  const rows = tableFromSnapshot(snapshot, table);
  const page = filteredRows(rows, url);
  return json({
    ok: true,
    schema: "OMEGA_APPLIED_CALCULUS_ANALYSIS_R193",
    release: DRIVE_CORPUS_RELEASE_R193,
    table: tableId,
    formula: MODE188_FORMULA_R193,
    total: page.total,
    ...
    offset: page.offset,
    limit: page.limit,
    rows: page.rows.map((row, index) => ({ corpusIndex: page.offset + index, row, mode188: mode188DerivedR193(row) })),
    boundary: "Derived analysis is evidence-bound to the embedded Drive row. R193 preserves source decisions and does not fabricate missing thresholds or measurements.",
    canonicalMutation: false,
  });
}

function systemHtmlR193(): string {
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>OMEGA ONE SYSTEM — R193</title>
<style>
...
</style></head><body><div class="app"><aside><div class="brand">OMEGA ONE SYSTEM</div><div class="rev">R193 DRIVE CORPUS CONVERGENCE</div><nav class="nav"><a href="/">Canon</a><a href="/instrument">Instrument</a><a href="/truth">Truth</a><a href="/fabric">Fabric</a><a href="/clouds">Clouds</a><a href="/sai">SAI</a><a href="/compute">Compute</a><a href="/validate">Validate</a><a href="/federation">Federation</a></nav></aside><main>
...
<div class="card"><div class="kicker">Applied calculus</div><div class="law">S188=CΩ/(Λ+q+0.35Λq+0.05)</div><p class="muted">R193 applies the stability index only where the recovered row actually supplies continuity, burden and contradiction. Missing thresholds remain missing rather than being invented.</p></div>
...
async function loadManifest(){const {data}=await getJson('/api/system/r193/manifest');const reg=(data.corpus?.tables||[]).find(x=>x.id==='registry');$('registryCount').textContent=reg?.rows??'—'}
async function loadStatus(){const {data}=await getJson('/api/system/r193/status');$('overall').textContent=data.ok?'LIVE CORE + CORPUS VERIFIED':'PARTIAL / PROOF HOLD';$('overall').style.borderColor=data.ok?'var(--ok)':'var(--warn)';$('probes').innerHTML=(data.probes||[]).map(p=>'<div class="status-row"><span class="dot '+(p.ok?'ok':'bad')+'"></span><span>'+esc(p.id)+'<div class="muted">'+esc(p.route)+'</div></span><span>'+esc(p.status)+'</span></div>').join('')||'No probe data'}
...
async function loadTable(q=''){const {data}=await getJson('/api/system/r193/'+current+'?limit=100&q='+encodeURIComponent(q));renderRows(data)}
async function analyze(){const q=$('search').value;const {data}=await getJson('/api/system/r193/analyze?table='+encodeURIComponent(current)+'&limit=100&q='+encodeURIComponent(q));$('tableMeta').textContent='Mode188 analysis · '+data.matched+' matched / '+data.total+' total';const rows=(data.rows||[]).map(x=>({...x.row,Mode188Score:x.mode188?.score,Mode188Applicable:x.mode188?.applicable,SourceDecision:x.mode188?.sourceDecision}));renderRows({label:'Applied calculus '+current,matched:data.matched,total:data.total,rows})}
...
$('searchBtn').onclick=()=>loadTable($('search').value);$('analyzeBtn').onclick=analyze;$('clear').onclick=()=>{$('payload').value='';$('result').textContent='No execution yet.'};$('run').onclick=async()=>{const op=$('operation').value;const raw=$('payload').value.trim();let body={operation:op};if(op.startsWith('ai.')||op==='sai.query')body.prompt=raw;else{try{body.input=raw?JSON.parse(raw):{}}catch(e){$('result').textContent='Input must be valid JSON for compute operations.';return}}$('result').textContent='Executing…';const {data}=await getJson('/api/system/r193/execute',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});$('result').textContent=JSON.stringify(data,null,2)};
buildTabs();loadManifest();loadStatus();loadTable();setInterval(loadStatus,30000);
</script></body></html>`;
}

export async function handleDriveCorpusSystemR193(request: Request, env: any, ctx: any, canonicalFetch: CanonicalFetch): Promise<Response | null> {
  const url = new URL(request.url);
  const path = url.pathname.replace(/\/$/, "") || "/";
  const inScope = path === "/system" || path.startsWith("/api/system/r193");
  if (!inScope) return null;
  if (request.method === "OPTIONS") return json(null, 204);
  try {
    if (path === "/system" && request.method === "GET") {
      return new Response(systemHtmlR193(), { headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" } });
    }
    if (path === "/api/system/r193/manifest" && request.method === "GET") return json(await corpusManifestR193(env));
    if (path === "/api/system/r193/status" && request.method === "GET") return json(await liveStatusR193(request, env, ctx, canonicalFetch));
    if (path === "/api/system/r193/execute" && request.method === "POST") return executeR193(request, env, ctx, canonicalFetch);
    if (path === "/api/system/r193/analyze" && request.method === "GET") return analyzeResponseR193(url);
    const tableId = path.startsWith("/api/system/r193/") ? path.slice("/api/system/r193/".length) : "";
    if (request.method === "GET" && CORPUS_TABLES.some(table => table.id === tableId)) return tableResponseR193(tableId, url);
    if (request.method !== "GET" && request.method !== "POST") return json({ ok: false, code: "METHOD_NOT_ALLOWED", allowed: ["GET", "POST"] }, 405);
    return json({
      ok: false,
      code: "R193_SYSTEM_ROUTE_NOT_FOUND",
      routes: ["/system", "/api/system/r193/manifest", "/api/system/r193/status", "/api/system/r193/execute", "/api/system/r193/analyze", ...CORPUS_TABLES.map(table => `/api/system/r193/${table.id}`)],
      canonicalMutation: false,
    }, 404);
  } catch (error) {
    return json({
      ok: false,
      code: "R193_SYSTEM_FAILURE",
      error: error instanceof Error ? error.message : String(error),
      corpusSha256: DRIVE_CORPUS_SNAPSHOT_SHA256_R193,
      canonicalMutation: false,
    }, 500);
