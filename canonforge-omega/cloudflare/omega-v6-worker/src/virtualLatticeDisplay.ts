...
export const VISUAL_DELIVERY_RELEASE = "r141-visual-delivery-correctness";
...
export const VISUAL_DELIVERY_BOUNDARY =
  "R141 preserves the R140 individualized root Field and adds observable runtime-integrity proof beneath heartbeatTruth. Parent, Interaction, Scar, Continuity, Compression, Skin, Interpretation and Behavior continue shaping the 20,736 control individuals across the 12^10 logical address hierarchy. Delivery now carries an exact visual release marker and a browser-side liveness monitor for UTC, phase, relativity depth and skin individuality. This remains presentation/model-only: it does not mutate canonical state, alter OmegaRuntime authority, weaken Hybrid heartbeat truth, change Genesis role separation, promote animation to evidence, or treat representational 12^n shells as physical dimensions. It does not claim a physical 61.9-billion-pixel panel; 61,917,364,224 is a logical hierarchical display-address capacity.";
...
async function stampDeliveredVisual(response: Response): Promise<Response> {
  ...
  headers.set("x-omega-visual-contract", "root+skin+phase+integrity-v1");
  ...
  if (!html.includes("omegaVisualDeliveryRelease")) {
    ...
    html = html.replace(/content="r1(?:3[789]|4[01])[^"]*"/g, `content="${VISUAL_DELIVERY_RELEASE}"`).replace(/data-omega-visual-release="r1(?:3[789]|4[01])[^"]*"/g, `data-omega-visual-release="${VISUAL_DELIVERY_RELEASE}"`);
  }
