...
def release_lineage_guard() -> dict:
    ...
    The canonical branch is not currently server-side protected. For canonical
    GitHub Actions push promotion, require a two-parent merge commit whose first
    parent is exactly the branch head reported by the push event before the merge.
    This is deliberately not enforced for PR validation or local checks.
    """
    enforced = (
